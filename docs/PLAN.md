...
`ResolveStandards` · `AssembleWorksheet` · `BuildVariants(student/teacher)` · `ValidateWorksheet` · `RenderPdf` / `RenderPng` · `EditWorksheet` · `SaveDocument`/`OpenDocument`(E1 워크스페이스) · `RenderEditorShell`(E2/E3 에디터 합성) · `ExportDocument`(E6 — unsafe fail-closed PDF 내보내기). 어떤 유스케이스도 Chrome·gepai·파일시스템을 직접 부르지 않는다.

### 4.2b 에디터(E0~E6) 아키텍처 — HANDOFF-editor-workspace.md §12 상세
...
- **내보내기 통합(E6 — 로드맵 완결)**: "저장이 곧 게이트" — export·정밀 미리보기·용지 변경 전부 저장본만 대상(두 번째 진실 소스 금지). 신규 `ExportDocument`(서버 `POST /export`·CLI `doc export` 동일 코어)가 **`meta.unsafe` 를 fail-closed 로 승격** — student PDF 거부(스테일 물리 제거, SaveDocument 보류와 대칭)·teacher 는 항상 산출. 정밀 미리보기 = `GET /preview.png`(백그라운드 Chrome, scale:2 핀, 첫 페이지, in-flight 1개). 포맷 프리셋(§3.3) = paper.js `PAPER_PRESETS` 4종 + 고급 → `POST /paper`(no-op 가드·SaveDocument 경유) → 셸 재로드 = 재페이지네이션(E3 iframe 유지의 명시적 예외). "A3 접이"는 A3 가로 단일 시트까지 — imposition 은 후속.
