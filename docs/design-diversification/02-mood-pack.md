...
- **P2-c-2 — ✅ 완료(`622f930`, 세 무드 과감 재설계)**: 사용자 피드백 "무드가 거의 안 구분됨" 반영 —
  토큰을 크게 벌리고(제목 16/24/20pt · 본문 9/11.5/10pt · 모서리 0/22/0 · 리듬 2/6/3mm · 선색 회색/탄색/슬레이트)
  헤더 변형을 무드별로 다르게: exam=밑줄(border-bottom) · soft=큰 둥근 밴드(border-radius) · angular=좌측
  강조바(border-left). `mood.render.test.js` 를 3무드 렌더로 확장(헤더 시그니처별 검증).
- **관련 무드 밖 수정(`bd640be`, migrate/render 회귀 2건)**: 무드 미리보기 검토 중 발견한 마이그레이션
  렌더 버그 — ①복합 header(학년/반/이름·단원 줄)가 title 로 병합돼 제목에 흡수되던 것 → richtext 무손실
  보존, ②subq/section-heading(level 2)이 title-box(큰 제목 양식)로 그려지던 것 → `<h2 class="sec">` 작은
  섹션 헤딩. acceptance 페이지수 불변·parity·개체트리 스키마 무변경.
- **P2-c-3 — ✅ 완료(`3a2545b`, 표 밀도 변형)**: 헤더에 이은 두 번째 변형 차원 — 데이터 표(`.obj-table
  th/td`)·라벨값 표(`.lv-table td`) 셀 여백을 무드별로: exam=조밀(2px 6px·min-h 18px) / soft=여유
  (9px 14px·min-h 32px) / angular=크리스프(4px 10px). 무드 지정 문서에만 로드 → 비무드 무영향.
- **P2-c-4 — ✅ 완료(`d784034`, 섹션 헤딩·지시문 모티프 통일)**: 각 무드의 헤더 모티프를 `h2.sec`(문항
  소제목·섹션)·`.direct`(지시문)에도 확장 — exam=밑줄 / soft=둥근 밴드(--clite 배경+radius) / angular=좌측 바.
  헤더→섹션헤딩→지시문이 같은 모티프로 응집돼 무드별 일관 디자인 언어 완성.
- **P2-c-5 — ✅ 완료(`c089d06`, 정식 무드 팩 확장 → 5무드 서랍)**: PLAN "디자인 방향 서랍" 완성 —
...
- **P2-c-7 — ✅ 완료(`2f2eb17`, 강한 색 표 헤더 톤)**: 루브릭·데이터·어휘·메모 헤더(`--cstrip` 흰글자)를
  절제 무드(exam·wide)에서 옅게(`--clite`), soft/calm/angular 는 색밴드 유지. 조직자 헤더는 이미 옅어 대상 아님.
- **무드 구성요소 커버리지 정리**: 헤더·표밀도·섹션헤딩·지시문·강조상자·강한표헤더까지 무드로 변형됨.
  남은 것(조직자 셀 배경=이미 옅음, 별점=의미색 불변, pill/qnum=형태 정의)은 마진 낮음/위험 → **무드 축은
  여기서 충분**. 추가 변형이 필요하면 그때 개별 검토. 다음은 P3(용지 방향)·P4(목적→무드 자동선택) 권장.
