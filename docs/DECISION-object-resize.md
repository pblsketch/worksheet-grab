...
패커에 열 커서를 넣고() 단위 8건이 전부 초록인데도 2단 3자 파리티에서
**인쇄만 7쪽**이었다(모델 4쪽). 처음엔 측정 문제로 진단했으나, 실 Chrome 으로 재 보니 그게
아니었다:

\
 는 **컨테이너 높이가 정해져야** 열을 나눈다.  에 높이가 없어
다단 문서는 선언만 2단이고 출력은 1단이었다 — 모델이 2단을 가정하고 인쇄가 1단이었으니 4 vs 7.

**기존 테스트가 그 공백을 가리고 있었다.**  가 스스로
 를 주입해 2단을 재현했다(PAGED_PROXY).
테스트만 2단이고 산출물은 아니었다.

해소:  가 columns>1 에서만 (페이지 콘텐츠 박스 높이)를 방출하고
 의  가 소비한다(단단은 미방출 = 출력 바이트 불변). 두 페이지네이션
경로에  를 함께 배선했다. PAGED_PROXY 는 제거했다 — 이제 그 테스트가 진짜 산출물을
검증한다.

남은 미세 오차(기록): 측정이 flow 를 단일 논리 페이지에 펼치므로 열 경계마다 개체 하나의 높이가
0 으로 잘린다. 측정 중 열을 접어 없앨 수 있으나 2단·3단 어느 픽스처로도 페이지 귀속이 달라지지
않아(변이 실험) 검증 불가라 싣지 않았다 — 되살리는 방법은  머리말에.
