...
| 다단 열 간 재배분 | **절반** — 패커는 완료·검증, **배선은 보류**(아래) |

#### 다단: 패커는 됐고 측정이 남았다

`assignFlowToPages` 에 열 커서를 넣었다(`opts.columns`). `paper.css` 가 `column-fill:auto` +
자식 `break-inside:avoid` 라 브라우저가 좌열을 끝까지 채우고 블록을 쪼개지 않으므로, 열 단위
그리디 패킹이 그 동작과 1:1 대응한다. 단위 7건으로 고정했고 `columns` 미지정·불량 입력은 전부
단단으로 떨어져 **기존 산출은 바이트 동일**이다.

**그런데 호출부에는 아직 넘기지 않는다.** 2단 3자 파리티를 실제로 돌려 보니 편집기와 Chrome
페이지네이션은 서로 일치(4쪽)하는데 **인쇄 PDF 는 7쪽**이었다. 원인은 패커가 아니라 **측정**이다:
측정 렌더가 `renderMeta` 를 그대로 쓰므로 다단 문서는 2단 레이아웃에서 높이를 재는데, 그러면
개체 높이를 얻는 `next.top - cur.top` 델타가 열 경계에서 무의미해져(다음 열 첫 개체의 top 이 더
작다) 총 높이가 과소평가된다. 이 상태로 열 수만 넘기면 파리티가 **종전보다 나빠진다**
(종전 ≈8쪽 예측 · 지금 4쪽 · 실제 7쪽).

**다음 단계** — 측정을 "열 폭의 **단일 열**"로 바꾼다. 측정에 필요한 것은 열 기하가 아니라
*그 폭에서의 높이*뿐이다. 측정용 paper 를 `columns:1` + 좌우 여백을 넓혀 본문 폭을
`(W - mL - mR - (cols-1)·gap) / cols` 로 만들면 CSS 변경 없이 기존 paper 의미만으로 된다.
두 측정 경로(`PaginateObjectTree.execute` · `reflow.js buildMeasureHtml`)를 **함께** 바꿔야 한다 —
한쪽만 바꾸면 하드 동치가 즉시 깨진다. 검증은 `editor-print-parity.render.test.js` 에 2단 픽스처를
추가해 인쇄 PDF 쪽수까지 대조하면 된다(이번에 그 테스트로 문제를 잡았다).
