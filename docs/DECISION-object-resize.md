...
- ~~**파리티 하네스 보강(권고)**~~ → **완료(`3363202`)**. `test/helpers/renderParity.js` 가 두 모드
  렌더의 인라인 선언 목록을 비교하고, `test/unit/render-edit-print-parity.test.js` 가 **카탈로그 전
  타입 × 크기 필드 × 정렬**을 기계적으로 훑는다. 파리티 3건에도 가드 한 줄씩 넣었다.
  변이 실험으로 확인한 **실제 효과 범위**(권고 당시 예상보다 좁다): 신규 단위 스위트는 Chrome 없이
  0.7ms 만에 잡지만, 기존 파리티 픽스처(넘침·표)는 **여전히 못 잡는다** — 그 픽스처에 크기 필드를 가진
  개체가 없어 갈릴 거리가 애초에 없기 때문이다. 즉 이득은 "파리티가 똑똑해진 것"이 아니라 **손으로
  현실적 픽스처를 만들지 않아도 전 타입 조합이 매 `test:unit` 마다 검사된다**는 쪽이다.
- ~~범위 밖으로 둔 것은 §7 그대로(선행 결함 2건 · shape 클릭 가로채기 · 회전 OBB · 다단 열 재배분).~~
  → 2026-07-28 후속에서 4건 해소, 1건은 절반만 진행. 아래 참조.

### §7 잔여 결함 후속 (2026-07-28)

| 항목 | 상태 |
|---|---|
| ⠿ 가 `+` 에 덮임 | **해소** — 원인 2건(최초 장식이 레이아웃 확정 전 · 컨트롤 세로 공간 부족)을 각각 재장식 스케줄과 가로 분리로 |
| 미선택 float 손잡이 무반응 | **재현 안 됨** — 그 사이 다른 작업이 고쳤다(실측 확인, probe 2.5 갱신) |
| `shape` 클릭 가로채기 | **해소** — 박스 두 겹 `pointer-events:none` + 그려진 요소만 `visiblePainted` |
| 회전 개체 OBB · 손잡이 각도 | **해소** — 순수 규칙 2종을 SAT 로, 리사이즈를 로컬 축 + 중심 보정으로 |
| 다단 열 간 재배분 | **해소** — 렌더가 열을 안 나누던 것이 진짜 원인이었다(아래) |

#### 다단: 원인은 측정이 아니라 렌더였다

패커에 열 커서를 넣고(`opts.columns`) 단위 8건이 전부 초록인데도 2단 3자 파리티에서
**인쇄만 7쪽**이었다(모델 4쪽). 처음엔 측정 문제로 진단했으나, 실 Chrome 으로 재 보니 그게
아니었다:

```
columnCount: "2"   columnFill: "auto"     ← 선언은 멀쩡
sheetH: 2247px     ← A4 는 1123px. .sheet 가 min-height 라 그냥 늘어났다
distinctX: [340]   ← x 군집 1개 = 사실상 단단
```

`column-fill:auto` 는 **컨테이너 높이가 정해져야** 열을 나눈다. `.sheet-body` 에 높이가 없어
다단 문서는 선언만 2단이고 출력은 1단이었다 — 모델이 2단을 가정하고 인쇄가 1단이었으니 4 vs 7.

**기존 테스트가 그 공백을 가리고 있었다.** `columns.render.test.js` 가 스스로
`.sheet-body{ height: calc(var(--sheet-h) - 22mm) }` 를 주입해 2단을 재현했다(PAGED_PROXY).
테스트만 2단이고 산출물은 아니었다.

해소: `paper.js` 가 columns>1 에서만 `--sheet-colh`(페이지 콘텐츠 박스 높이)를 방출하고
`paper.css` 의 `.sheet-body` 가 소비한다(단단은 미방출 = 출력 바이트 불변). 두 페이지네이션
경로(`PaginateObjectTree.execute` · `reflow.applyReflow`)에 `columns` 를 함께 배선했다 —
한쪽만 켜면 하드 동치가 즉시 깨진다. PAGED_PROXY 는 제거했다: 이제 그 테스트가 주입 없이
진짜 산출물을 검증한다.

검증은 `editor-print-parity.render.test.js` 의 2단 픽스처가 맡는다(편집기 == Chrome == 인쇄 PDF
쪽수). 변이 4종(`.sheet-body` 높이 제거 · `--sheet-colh` 미방출 · 두 경로의 columns 미전달)이
전부 이 테스트에 잡힌다.

남은 미세 오차(기록): 측정이 flow 를 단일 논리 페이지에 펼치므로 열 경계마다 개체 하나의 높이가
0 으로 잘린다(`next.top - cur.top` 델타가 음수 → 상류에서 0 클램프). 측정 중에만 열을 하나로
접어 없앨 수 있고 실제로 만들어 배선까지 해 봤으나, 2단·3단 어느 픽스처로도 페이지 귀속이
달라지지 않아 변이 실험이 그 코드를 구분하지 못했다 — **검증할 수 없는 코드는 싣지 않는다.**
되살리는 방법과 근거는 `reflow.js` 머리말에 남겼다.
