...
- **학습목표(std-box.objectives, 2026-07-23 학습목표 표기 전환)**: 활동지 상단에는 성취기준 원문이
  ...
  표시한다(하위호환 — 기존 문서 무회귀).
- **근거 성취기준 표시(std-box.showStandards, 2026-07-28 기본값 전환)**: 활동지에는 학습목표만 싣는
...
## 에러 핸들링
- **레이아웃 전용 2종은 저작하지 않는다(2026-07-28 신설).** `spacer`(빈 공간)·`page-break`(페이지
...
- **크기·정렬 필드 3종도 저작하지 않는다(2026-07-28 신설).** `widthPct`(본문 폭 대비 %)·
