...
- **색 팔레트(교과와 분리)**: `themeName` 기본은 `neutral`(slate) — **교과가 색을 강제하지 않는다.** 교사가 색 있는 룩을 명시 요청할 때만 `references/themes.md`의 팔레트 프리셋(ko/sci/social/english)을 `document.themeName`으로 고른다. 블록도 교과 전용이 아니므로 **활동(무엇을 시킬지)에 맞춰** 고른다(교과는 콘텐츠 맥락일 뿐, 색·블록의 하드 잠금이 아니다).
- **문체**: em대시(—)를 쓰지 않는다(한국어에서 부자연스럽다). 라벨·설명 구분은 `:`, 문장 중간 부연은 `,`나 문장 분리, 범위는 `~`(예: 1~3)로 쓴다.
- **레이아웃·여백**: 활동(subq)마다 위쪽 간격을 두어 활동 사이를 구분한다. 학생 입력란(`answer-line`은 `.tall`, `ansbox`는 `min-height`)과 조직자 높이를 넉넉히 잡아 페이지 하단 빈 공간을 줄인다. 지시문(`.direct`)에는 `<b>` 등 인라인 강조를 자유롭게 써도 된다(과거 단 나뉨 버그는 blocks.css 에서 해결됨).
