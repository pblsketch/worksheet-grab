...
# 3) 정적 검사(정답 누출·하드코딩 교과색·최소폰트)
node bin/worksheet-grab.js validate <in.html>
#   정답 누출 발견 시 종료코드 1(FAIL). 하드코딩 교과색은 경고.
...
node bin/worksheet-grab.js generate <학년교과> <주제> [--out <dir>] [--pdf]
#   예: generate 중2과학 광합성
...
node bin/worksheet-grab.js edit <base>.manifest.json "3번 문항 빼고 성찰 추가" [--out <dir>] [--no-render]
...
세부 수용·함정 기록은 `progress.txt`, 마일스톤 원장은 `.omc/ultragoal/`.
